...
async function init() {
  try {
    ...
    showToast('데이터 로드 실패. 콘솔을 확인하세요.', true);
  }
}
...
function toggleTagFilters() {
  ...
  if (dom.toggleTags) {
    dom.toggleTags.textContent = nowExpanded ? '접기' : '펼치기';
...
function renderGallery() {
  ...
  state.filteredItems.forEach((item) => {
    ...
    title.textContent = `사례 ${item.id}: ${item.title}`;
    ...
    if (item.examples && item.examples.length > 0) {
      ...
      exampleCount.textContent = `예시 ×${item.examples.length}`;
...
function updateResultStats() {
  ...
  const parts = [`총 ${filtered} / ${total}개 사례`];
  if (tags > 0) parts.push(`태그 ×${tags}`);
  if (hasSearch) parts.push('검색 적용됨');
  dom.resultStats.textContent = parts.join(' · ');
  //dom.resultStats.textContent = '';
}

function openModal(item) {
  // Ensure modal content starts at top on each open
  const modalContent = dom.modal.querySelector('.modal-content');
  if (modalContent) {
    modalContent.scrollTop = 0;
  }

  dom.modalTitle.textContent = `사례 ${item.id}: ${item.title}`;
  // Build source + model inline meta
  const metaParts = [];
  if (item.source && item.source.url) {
    metaParts.push(`출처: <a href="${item.source.url}" target="_blank" rel="noopener">${item.source.name}</a>`);
  } else if (item.source && item.source.name) {
    metaParts.push(`출처: ${item.source.name}`);
  }
  if (item.model) {
    metaParts.push(`모델: ${item.model}`);
  }
  ...
  // Double-check after render in case layout changed
  if (modalContent) {
    modalContent.scrollTop = 0;
  }
}
...
function closeModal() {
  ...
  // Also reset scroll so next open always starts at top
  const modalContent = dom.modal.querySelector('.modal-content');
  if (modalContent) {
    modalContent.scrollTop = 0;
  }
...
function renderModalPrompts(prompts) {
  ...
  const heading = document.createElement('h3');
  heading.textContent = '프롬프트';
  dom.modalPrompts.appendChild(heading);

  prompts.forEach((prompt, index) => {
    dom.modalPrompts.appendChild(createPromptBlock(prompt, index + 1, '프롬프트'));
  });
}
...
function renderModalExamples(examples) {
  ...
  const heading = document.createElement('h3');
  heading.textContent = '예시 프롬프트';
  dom.modalExamples.appendChild(heading);

  examples.forEach((example, index) => {
    dom.modalExamples.appendChild(createPromptBlock(example, index + 1, '예시'));
  });
}
...
async function copyPrompt(text, button) {
  try {
    ...
    showToast('대체 방식으로 복사되었습니다', false);
  }
}
...
function animateCopyButton(button, withWarning = false) {
  button.textContent = withWarning ? '복사됨 (대체)' : '복사됨';
